...
var configs = {
  ...
  sysconfdir: {
    findpattern: 'device_classes.xml',
...
function getConfigItem(item) {
  ...
  if (usepkgconfig) {
    return (cp.execSync(util.format("pkg-config --variable=%s libopenzwave",
      item)));
  } else {
    // 2. try using find
    ...
    for (var i in configs[item].locations) {
      ...
      var cmd = util.format(
        "find %s -type d ! -perm -g+r,u+r,o+r -prune -o -name %s", loc, fp);
      try {
...
Object.keys(configs).forEach(function(item) {
  var buf = getConfigItem(item);
  if (buf) {
    var arr = buf.toString().split('\n');
    paths[item] = path.dirname(arr[0]);
  } else {
    // when everything else fails...
    paths[item] = configs[item].default;
    //console.log('ITEM %s NOT FOUND!', item);
  }
})
